...
namespace hospital
{
    ...
    {
        private string position_username;
        private string position_role;
        String MySQLConn = "";
        MySqlConnection conn;
        MySqlCommand command;
        DataTable table;
        private string sqlquery = "SELECT * FROM tbposition ORDER BY id DESC";
        Boolean buttonSave, buttonEdit, buttonRemove;
        public FormPosition(String position_username, String position_role)
        {
            InitializeComponent();
            this.Text = "Position";
        ...
        {
            FormAdmin formAdmin = new FormAdmin(position_username, position_role);
            formAdmin.Show();
            this.Hide();
        }
        ...
        private void Refresh()
        {
            ...
            {
                conn.Open();
                MySqlCommand command = new MySqlCommand("SELECT * FROM tbposition WHERE active = 1 ORDER BY id DESC", conn);
                ...
                MySqlCommand command_id = new MySqlCommand("SELECT id FROM tbposition ORDER BY id DESC LIMIT 1", conn);
            ...
            if (btnSave.Text == "Save")
            {
                ...
                try
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        ...
                        {
                            MessageBox.Show("This position already exist. Please try again!!!", "Fail", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            conn.Close();
                            return;
                        }
                    }
                    ...
                    string query = "INSERT INTO tbposition(id, name) VALUES (@id, @name)";
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                String updateQuery = "UPDATE tbposition SET active = @newValue WHERE id = @id";
        ...
        private void TrackUserAction(string userAction)
        {
            try
            {
                using (conn = new MySqlConnection(MySQLConn))
                {
                    ...
                    command.Parameters.AddWithValue("uForm", "Position");
                    command.Parameters.AddWithValue("uID", "");
                    command.Parameters.AddWithValue("uName", position_username);
                    command.Parameters.AddWithValue("uRole", position_role);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells[1].Value.ToString().Equals(txtName.Text))
                    {
                        MessageBox.Show("This position already assists. Please try again!!!", "Fail", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        conn.Close();
                        return;
                    }
                }
                ...
                String updateQuery = "UPDATE tbposition SET name = @newName WHERE id = @id";
