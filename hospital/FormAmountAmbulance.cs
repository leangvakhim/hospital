...
namespace hospital
{
    public partial class FormAmountAmbulance : Form
    {
        private string amountambulance_username;
        private string amountambulance_role;
        String MySQLConn = "";
        MySqlConnection conn;
        MySqlCommand command;
        DataTable table;
        private string sqlquery = "SELECT * FROM tbamountambulance ORDER BY id DESC";
        Boolean buttonSave, buttonEdit, buttonRemove;

        private void btnReturn_Click(object sender, EventArgs e)
        {
            FormAdmin formAdmin = new FormAdmin(amountambulance_username, amountambulance_role);
            formAdmin.Show();
            this.Hide();
        }
        ...
        public FormAmountAmbulance(String amountambulance_username, String amountambulance_role)
        {
            InitializeComponent();
            this.Text = "Position";
            this.amountambulance_username = amountambulance_username;
            this.amountambulance_role = amountambulance_role;
        ...
        private void FormAmountAmbulance_Load(object sender, EventArgs e)
        {
            Refresh();
        }
        ...
        private void Refresh()
        {
            ...
            {
                conn.Open();
                MySqlCommand command = new MySqlCommand("SELECT * FROM tbamountambulance WHERE active = 1 ORDER BY id DESC", conn);
                ...
                MySqlCommand command_id = new MySqlCommand("SELECT id FROM tbamountambulance ORDER BY id DESC LIMIT 1", conn);
            ...
            if (btnSave.Text == "Save")
            {
                ...
                try
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        ...
                        {
                            MessageBox.Show("This ambulance's name already exist. Please try again!!!");
                            conn.Close();
                            return;
                        }
                    }
                    ...
                    string query = "INSERT INTO tbamountambulance(id, name) VALUES (@id, @name)";
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells[1].Value.ToString().Equals(txtName.Text))
                    {
                        MessageBox.Show("This ambulance's name already assists. Please try again!!!", "Fail", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        conn.Close();
                        return;
                    }
                }
                ...
                String updateQuery = "UPDATE tbamountambulance SET name = @newName WHERE id = @id";
        ...
        private void FormAmountAmbulance_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
        ...
        private void TrackUserAction(string userAction)
        {
            try
            {
                using (conn = new MySqlConnection(MySQLConn))
                {
                    ...
                    command.Parameters.AddWithValue("uForm", "AmountAmbulance");
                    command.Parameters.AddWithValue("uID", "");
                    command.Parameters.AddWithValue("uName", amountambulance_username);
                    command.Parameters.AddWithValue("uRole", amountambulance_role);
